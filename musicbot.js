...
client.on("message", msg => {
	...
	if (msg.isMentioned(client.user.id)) {return msg.reply("Use " + guildPrefix + "help for commands.")}
    ...
    if (command === "play" || command === "p") {
	  ...
	  if (!_guild.audio.voice === msg.member.voiceChannel) return msg.channel.sendMessage("You need to be in the JukeBot's voicechannel to queue.")
      const sMsg = msg.channel.sendMessage("Gathering video metadata...")
      var query = args
      if (query.lastIndexOf("=") !== -1) {query = query.substring(query.lastIndexOf("=") + 1)}
  ...
  if (command === "now") {
		if (_guild.audio.playing) {
			...
			msg.channel.sendMessage("There is nothing playing.")
		}
	}
  ...
  if (command === "op") {
		if (msg.author.id !== config.ownerID) return msg.channel.sendMessage("You don't have access to this command.")
			...
			if (idparam.startsWith("-")) {
				if (!config.adminlist.includes(idparam.substring(1))) {return msg.channel.sendMessage("That user is not an admin!")}
				if (config.adminlist.includes(msg.author.id)) return msg.channel.sendMessage("You cannot de-op other admins.")
				config.adminlist.splice(config.adminlist.indexOf(idparam.substring(1)), 1)
			} else {
				if (config.adminlist.includes(idparam)) {return msg.channel.sendMessage("That user is already an admin!")}
